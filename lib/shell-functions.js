...
swt() {
    case "$1" in
        c|create)
            ...
            ;;
        d|delete)
            # Create a temporary file to capture stderr marker
            tmpfile=$(mktemp)
            # Run the command, capturing stderr to check for marker but still showing prompts
            command swt "$@" 2> >(tee "$tmpfile" | grep -v "__SWT_DELETED_FROM_INSIDE__" >&2)
            # Check if we deleted from inside and need to change directory
            new_dir=$(grep -o "__SWT_DELETED_FROM_INSIDE__:.*" "$tmpfile" 2>/dev/null | sed 's/__SWT_DELETED_FROM_INSIDE__://')
            rm -f "$tmpfile"
