...
async function deleteWorktreeByPath(worktreePath, mainRepo, options = {}) {
  ...
  try {
    ...
    if (fs.existsSync(worktreePath)) {
      throw new Error('Worktree directory still exists after removal attempt');
    }
    ...
    if (inTargetWorktree) {
      ...
      // We'll output it at the very end so it doesn't interfere with prompts
      process.on('exit', () => {
        console.log(`\n__SWT_DELETED_FROM_INSIDE__:${mainRepo}`);
      });
    } else {
