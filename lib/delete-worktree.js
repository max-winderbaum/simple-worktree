...
async function deleteWorktreeByPath(worktreePath, mainRepo, options = {}) {
  ...
  try {
    ...
    try {
      ...
      // Check if directory still exists and try to remove it
      if (fs.existsSync(worktreePath)) {
        try {
          fs.rmSync(worktreePath, { recursive: true, force: true });
          console.log(chalk.green('✔ Directory removed manually'));
        } catch (rmError) {
          throw new Error(`Failed to remove directory: ${rmError.message}`);
        }
      }
    }
    ...
    if (fs.existsSync(worktreePath)) {
      // Try one more time with fs.rmSync
      console.log(chalk.yellow('Directory still exists, attempting final cleanup...'));
      try {
        fs.rmSync(worktreePath, { recursive: true, force: true });
        console.log(chalk.green('✔ Directory removed in final cleanup'));
      } catch (finalError) {
        console.log(chalk.red(`Failed final cleanup: ${finalError.message}`));
